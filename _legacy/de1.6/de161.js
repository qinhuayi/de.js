/// de.js ver1.6.1dev (2017-11-15)
/// ver 1.6.1dev {2017-11-19}
///   1. Add function de.parent(specifies);
///   2. Upgrade _tags(specifies) to support property value filter '[:name=value]';
/// ver 1.6 {2016-11-19}
...
(function (window, document, undefined) {
    var readyList = [],
        ...
        _safeExprssion = function(express) {
            var me = this,
                ...
                getSafeExpression = function (expr, rep) {
            ...
            me.Expreession = express;
            me.Replacement = rep;
            me.SafeExpression = getSafeExpression(express, rep);
            me.Restore = restore;
        },
        _exam = function (e, tagName, className, express, safeexpr) {
            var examClass = function (e, name) {
                ...
                examAttrs = function (e, exprss) {
                    ...
                    var pass = true,
                        exprs = exprss.indexOf(safeexpr.Replacement.outsideComma) >= 0 ? exprss.split(safeexpr.Replacement.outsideComma) : exprss.split(',');
                    exprs.each(function (i, expr) {
                        pass = pass && examAttr(e, expr.trim());
                    });
                    return pass;
                };
            return e.tagName.toUpperCase() == tagName.toUpperCase() && (typeof (className) !== 'string' || !className || examClass(e, className)) && (typeof (express) !== 'string' || !express || examAttrs(e, express));
        },
        _tags = function (e, specifies) {
            var elements = [],
                ...
                rep = createReplacement(),
                replaceSafeExpression = function (expr, rep) {
                    var matchs = expr.match(/('[^']+')|("[^"]+")/g);
                    if (matchs) {
                        matchs.each(function (i, m) {
                            var str = m.substr(1, m.length - 2).replace(/\,/g, rep.insideComma).replace(/\\/g, rep.slash).replace(/\//g, rep.backSlash).replace(/\[/g, rep.leftSquareBracket).replace(/\]/g, rep.rightSquareBracket).replace(/'/g, rep.singleQuotes).replace(/"/g, rep.doubleQuotes);
                ...
                restore = function (saveExpr, rep) {
                    var reg = function (str) {
                        return new RegExp(str, 'g');
                    };
                    return saveExpr.replace(reg(rep.insideComma), ',').replace(reg(rep.outsideComma), ',').replace(rep.slash, '\\').replace(rep.backSlash, '/').replace(reg(rep.leftSquareBracket), '[').replace(reg(rep.rightSquareBracket), ']').replace(reg(rep.singleQuotes), "'").replace(reg(rep.doubleQuotes), '"');
                },
                safeSpecifies = replaceSafeExpression(specifies, rep).split(','),
                ...
                getLevelElements = function (e, lspec) {
                    ...
                    if (/^(\\?\w+)(\.\w+)?(\[[^\]]+\])?$/ig.test(lspec.trim())) {
                        ...
                        for (var i = 0; i < arr.length; i++) {
                            if (_exam(arr[i], tagName, className, express, rep)) {
        ...
        _findParent = function (e, specifies) {
            var count = 0,
                node = e.parentNode || e,
                match = function (e, spec) {
                    if (/^(\w+)(\.\w+)?(\[[^\]]+\])?$/ig.test(spec.trim())) {
                        var tagName = RegExp.$1,
                            className = !!RegExp.$2 ? RegExp.$2.replace('.', '') : '',
                            express = RegExp.$3;
                        return _exam(e, tagName, className, express);
                    }
                    return false;
                };
            while (++count < 256) {
                if (match(node, specifies)) {
                    return _de(node);
                }
        ...
        _de = function (e, doc) {
            ...
            e.parent = function (specifies) {
                return _findParent(e, specifies);
            };
            e._de = true;
            return e;
        },
